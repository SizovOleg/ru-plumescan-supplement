...
Именно эта проверка обосновывает единую метку `algorithm_version`: годовые
коллекции собирались в разное время, и исходные метки сборки, сохранённые в
`algorithm_version_recorded`, расходятся.
...
def main() -> int:
    ...
    # Правило проверяется пособытийно, а не по метке: метки сборки в
    # algorithm_version_recorded расходятся, а правило должно быть одно.
