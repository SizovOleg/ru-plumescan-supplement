...
Описание всех 60 полей — в [словаре](data/DATA_DICTIONARY.ru.md). Таблицы статьи в CSV и порядок их пересчёта — в [`data/tables/`](data/tables/README.ru.md).
...
**Версия кода.** Каталог собран версией 3.1.4; код в `code/` — версия 3.2.0, в которой эталон фона кольца стал именованным параметром конфигурации (пресет). Пресет `default` с эталоном `industrial_buffers` воспроизводит опубликованный каталог: у 121 из 122 событий максимальная z-оценка совпадает до сотых, расхождение — у CH4-WSP-006 (сверку выполняет `code/py/analysis/step8e_event_ring_bias.py`). Однако `params_hash` пересобранного каталога будет другим, потому что ключи эталона входят в снимок параметров (для 2021 г. — `71d2a7c0…` вместо опубликованного `1d909d3a…`).
